...
public class EffectStatus {
    ...
    public void retryActiveEffects(BonusTotemData data) {
        for (AbstractEffect effect : notActivedEffects) {
            if (effect.getCondition().getAllBoolean(effect.player, data)) {
                effect.addPlayerStat();
                addAcvtiedEffects(effect);
            }
        }
        for (AbstractEffect effect : activedEffects) {
            if (!effect.isAlwaysCheckCondition() && !effect.getCondition().getAllBoolean(effect.player, data)) {
